...
const RecipesRestaurant = () => {
    ...
    if(resInfo?.length === 0) return <HomeShimmer />
  return (
    ...
    <div className='mt-20 xs:mt-28 mb-12'>
        ...
        <div className="flex flex-wrap justify-center" ref={resContainerRef}>
            {
                resList?.map((restaurant, idx) => idx > 2 && <Link style={{ color: "black", textDecoration: 'none' }} key={restaurant?.card?.card?.info?.id} to={"/restaurants/" + restaurant?.card?.card?.info?.id}>
                    {
                        ("aggregatedDiscountInfoV3" in restaurant?.card?.card?.info) ? <ResturentCardVeg resData={restaurant?.card?.card} /> : <ResturentCard resData={restaurant?.card?.card} />
                    }
                </Link>)
            }
        </div>
